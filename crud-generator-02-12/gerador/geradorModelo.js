...
class GeradorCRUD 
{
    ...
    gerarModel() //R
    {
        ...
        this.codigoFonte += "   constructor (" + vetAtributos + ")" + " {\n";
    ...
    gerarController()
    {
        ...
        criarFuncaoSalvar(this);
        criarFuncoesCSV(this);

        function criarFuncaoSalvar(esseGerador) 
        {
            esseGerador.codigoFonte += `function salvar() 
            {
            ...
            for(let i = 0; i < esseGerador.atributosObjetos.length; i++)
            {
                const atual = esseGerador.atributosObjetos[i];

                esseGerador.codigoFonte += `
                const ${atual.nome} = ${corretorDeTipo()} document.getElementById(${atual.idHTML});
                `
            }
        }

        function criarFuncoesCSV(esseGerador) 
        {;
        }
