...
//// nome classe minúsculo
let pet = null; //variavel global 
// let entidadeClasseGlobal (substituir todos pet)
...
function procure() {
    ...
    if (id) { // se digitou um Id
        ...
        if (entidadeAtualGlobal) { //achou na lista
            mostrarDadosPet(entidadeAtualGlobal);
...
function salvar() {
    ...
    //// if (id && dado1 && dado2 && !isNaN(dadoNumerico)...
    if (id && nome) {// se tudo certo 
        switch (oQueEstaFazendo) {
            case 'inserindo':
                //// new NomeGlobal (atributos...)
                entidadeAtualGlobal = new Pet(id, nome);
            ...
            case 'alterando':
                ...
                //// lista + pet -> listaGlobal + entidadeClasseGlobal
                listaDados[entidadeAtualGlobal.posicaoNaLista] = petAlterado;
...
function prepararESalvarCSV() { //gera um arquivo csv com as informações da lista. Vai enviar da memória RAM para dispositivo de armazenamento permanente.
    ...
    let textoCSV = "";
    for (let i = 0; i < listaDados.length; i++) {
        const linha = listaDados[i]; //variavel linha contem as informações de cada pet
        textoCSV += linha.id + ";" +
            linha.nome + "\n";
    }
        ...
        for (let i = 0; i < linhas.length; i++) {
            ...
            if (linha) { //verifica se a linha não está vazia
                ...
                if (dados.length === 2) { //verifica os seis campos
                    ...
                    //// todos os dados + nomes das propriedades
                    listaDados.push({
                        id: dados[0],
                        nome: dados[1]
                    });
