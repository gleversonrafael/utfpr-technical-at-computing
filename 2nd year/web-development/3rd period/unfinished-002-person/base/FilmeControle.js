...
function previewImagem() {
    ...
    if (inputFiles.length > 0) {
        ...
        document.getElementById('imgPessoa').src = url;
...
async function salvar() {
    let id_pessoa = document.getElementById("inputId_pessoa").value;
    ...
    const nome_mae = document.getElementById("inputNome_mae").value;
    ...
    try {
        if (oQueEstaFazendo === 'inserindo') {
            await fetch(`${URL_API}/pessoa`, { 
                method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(dadosPessoa) 
            });
            ...
            await fetch(`${URL_API}/pessoa/${id_pessoa}`, 
                { 
                    method: 'PUT', 
